...
genai.configure(api_key="AIzaSyDCD2hQ5WnSiOO5xs8Meduk1_xk1jOEk5M")  # Replace with your Gemini API key
...
def load_data():
    try:
        df = pd.read_csv('my_data.xlsx')  # Replace with your dataset file name
...
# App Header
st.markdown('<h1 class="chat-font">🤖 Sanjana Chatbot</h1>', unsafe_allow_html=True)
st.markdown('<h3 class="chat-font">Ask me anything, and I\'ll respond as Sanjana!</h3>', unsafe_allow_html=True)
st.markdown("---")
...
def generate_refined_answer(query, retrieved_answer):
    prompt = f"""You are Sanjana. Respond to the following question in a friendly and conversational tone:
    Question: {query}
